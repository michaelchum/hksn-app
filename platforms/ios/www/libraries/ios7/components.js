...
prx.types.ios7_button_bg = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(item.iconpos != "none") {
			var icon = '<img class="ios7-button-icon" src="'+prx.componentsHelper.getAssetUrl(item.img)+'">';
		}
...
prx.types.ios7_alert = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.buttons, function(i,elm) {
			...
			cR += '<div id="'+_id+'-buttons-'+i+'" class="ios7-alert-button dynamic-property liveUpdate-borderColor liveUpdate-textColor-'+i+'" data-dynamic-property-index="'+i+'" style="'+_props+' color: '+prx.utils.getColor(elm.textColor)+';">';
			cR += '<span data-editableproperty="text" data-dynamic-property-index="'+i+'">' + elm.text + '</span>';
			cR += '</div>';
		});
		cR += '</div>';
